<template>
  <div v-if="movie" class="max-w-7xl mx-auto px-4 my-10">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm text-zinc-300">
      ...
      <div><strong class="text-white">Idioma Original:</strong> {{ movie.original_language?.toUpperCase() }}</div>

      <div v-if="movie.production_countries?.length">
        <strong class="text-white">País:</strong>
        {{ (movie.production_countries as any[]).map((c) => c.name).join(', ') }}
      </div>

      <div v-if="movie.budget">
        <strong class="text-white">Orçamento:</strong> US$ {{ movie.budget.toLocaleString() }}
      </div>

      <div v-if="movie.revenue">
        <strong class="text-white">Bilheteria:</strong> US$ {{ movie.revenue.toLocaleString() }}
      </div>

      <div v-if="movie.production_companies?.length">
        <strong class="text-white">Produtora(s):</strong>
        {{ (movie.production_companies as any[]).map((c) => c.name).join(', ') }}
      </div>
    </div>
  </div>
</template>
